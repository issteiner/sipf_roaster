...
uint8_t string2int(char *numberStr) {
    uint8_t result = 0;
    uint8_t index = 0;
...
uint8_t getParams(char *readLine, char *params) {
    uint8_t length = 0;
    while (*readLine != ';' && *readLine != 0x0) readLine++;
...
uint8_t getBeanTemperature() {
    ...
    return convert2Celsius(result / 2);
}
...
void main(void) {
    ...
    char readBuffer[15];
    char channels[5];
    volatile enum ArtisanCommand aCommand;
    uint8_t c = 0x40;
